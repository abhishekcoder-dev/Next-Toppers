  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="flex">
        ...
        {isMobileSidebarOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-40 md:hidden" onClick={() => setIsMobileSidebarOpen(false)}></div>
        )}
        
        {/* Sidebar */}
        <div className={`${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:static w-64 bg-gray-900 min-h-screen p-6 z-50 transition-transform duration-300 ease-in-out`}>
              ...
              <div className="bg-gray-900 rounded-lg border border-gray-700 overflow-hidden">
                ...
                <div className="md:hidden">
                  {students.filter(student => 
                    ...
                    return (
                      <div key={student} className={`p-4 border-b border-gray-700 ${blockedStudents.has(student) ? 'opacity-50' : ''}`}>
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h3 className="text-white font-semibold">{student}</h3>
                            <p className="text-gray-300 text-sm">{grades[originalIndex % grades.length]} • {courses[originalIndex % courses.length]}</p>
                          </div>
                          <span className={`text-sm ${blockedStudents.has(student) ? 'text-red-400' : 'text-green-400'}`}>
                            {blockedStudents.has(student) ? 'Blocked' : 'Online'}
                          </span>
                        </div>
                        <button 
                          onClick={() => toggleStudentBlock(student)} 
                          className={`w-full px-3 py-2 text-white rounded text-sm ${blockedStudents.has(student) ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}
                        >
